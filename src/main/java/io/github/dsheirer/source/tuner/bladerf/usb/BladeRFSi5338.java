...
public class BladeRFSi5338
{
    ...
    private static final int ENABLE_REGISTER_COUNT = 4;
    private final BladeRFNiosAccess mNiosAccess;
    private final int[] mEnableRegisterCache = new int[ENABLE_REGISTER_COUNT];
    private final boolean[] mEnableRegisterValid = new boolean[ENABLE_REGISTER_COUNT];
    private boolean mEnableRegisterReadable = true;
    ...
    {
        int enableReg = readEnableRegister(multiSynth);
        enableReg |= multiSynth.getEnable();
        writeRegister(ENABLE_BASE + multiSynth.getIndex(), enableReg);
        cacheEnableRegister(multiSynth.getIndex(), enableReg);
    ...
    private int readEnableRegister(MultiSynth multiSynth)
    {
        int index = sanitizeIndex(multiSynth.getIndex());
        if(!mEnableRegisterReadable)
        {
            return mEnableRegisterValid[index] ? mEnableRegisterCache[index] : multiSynth.getEnable();
        }

        try
        {
            int value = Byte.toUnsignedInt(readRegister(ENABLE_BASE + multiSynth.getIndex()));
            cacheEnableRegister(index, value);
            return value;
        }
        catch(SourceException se)
        {
            handleEnableRegisterFailure(index, multiSynth, se);
            return mEnableRegisterCache[index];
        }
    }

    private void cacheEnableRegister(int index, int value)
    {
        int sanitized = sanitizeIndex(index);
        mEnableRegisterCache[sanitized] = value & 0xFF;
        mEnableRegisterValid[sanitized] = true;
    }

    private void handleEnableRegisterFailure(int index, MultiSynth multiSynth, SourceException se)
    {
        if(mEnableRegisterReadable)
        {
            mEnableRegisterReadable = false;
            LOG.warn("Unable to read Si5338 enable register {}; assuming cached defaults", multiSynth.getIndex(), se);
        }

        if(!mEnableRegisterValid[index])
        {
            mEnableRegisterCache[index] = multiSynth.getEnable();
            mEnableRegisterValid[index] = true;
        }
    }
    ...
    private int sanitizeIndex(int index)
    {
        ...
        if(index >= ENABLE_REGISTER_COUNT)
        {
            return ENABLE_REGISTER_COUNT - 1;
        }
        return index;
    }
