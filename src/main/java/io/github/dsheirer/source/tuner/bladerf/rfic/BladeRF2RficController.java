...
public class BladeRF2RficController
{
    ...
    private static final int RX_CHANNEL_0 = 0;
    private static final int TX_CHANNEL_0 = 1;
    ...
    public int configureSampleRate(int channel, int requestedRate) throws SourceException
    {
        ...
        RxFilter currentRxFilter = readRxFilter();
        TxFilter currentTxFilter = readTxFilter();
        ...
        if(newLow)
        {
            ...
            {
                setRxFilter(RxFilter.DEC4);
                setTxFilter(TxFilter.INT4);
            }
        }
        ...
        if(oldLow && !newLow)
        {
            ...
            {
                setRxFilter(RxFilter.DEFAULT);
                setTxFilter(TxFilter.DEFAULT);
            }
        }
    ...
    private RxFilter readRxFilter() throws SourceException
    {
        long value = mNios.readRficCommand(RX_CHANNEL_0, BladeRFRficCommand.FILTER);
        return RxFilter.fromValue((int)value);
    }

    private TxFilter readTxFilter() throws SourceException
    {
        long value = mNios.readRficCommand(TX_CHANNEL_0, BladeRFRficCommand.FILTER);
        return TxFilter.fromValue((int)value);
    }

    private void setRxFilter(RxFilter filter) throws SourceException
    {
        mNios.writeRficCommand(RX_CHANNEL_0, BladeRFRficCommand.FILTER, filter.getValue());
    }

    private void setTxFilter(TxFilter filter) throws SourceException
    {
        mNios.writeRficCommand(TX_CHANNEL_0, BladeRFRficCommand.FILTER, filter.getValue());
    }
